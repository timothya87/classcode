...
sr = top_radiance(tau,Temp,height,T_surf,wavel,k_lambda)
...
def planckInvert(wavel,Blambda):
    ...
    Tbright =(c2/wavel)/(np.log(1+(c1*wavel**-5)/Blambda))
    return Tbright
...
if __name__=="__main__":
    ...
    for index,the_profile in enumerate(rad_profs):
        ...
        Brightnes = planckInvert(wavelengths,radiances)
        axis1.plot(wavelengths,Brightnes,label=str(dT_dz[index]*1.e3))
    axis1.set_title('Brightness Temps at TOA for 7 values of dT/dz (K/km)')
    axis1.set_ylabel('Brightness Temperature $(K)$')
